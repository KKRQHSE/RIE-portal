...
- **Centrale bibliotheek — browsertest + vervolg (open):** het hele centraal/lokaal-model nog met een echte admin- én KAM-login door de browser halen (admin: rubriek/vraag beheren; klant: koppelen, lokaal afwijken mét waarschuwing, een normwijziging zien doorwerken, inspectie starten vanuit de norm). Bekende vereenvoudiging: een door de admin gearchiveerde centrale vraag verdwijnt voor iedereen uit nieuwe inspecties, óók voor een bedrijf dat er lokaal van afweek (geen apart "norm liet dit vallen, je lokale kopie blijft"-signaal) — bewuste scope-keuze, later eventueel verfijnen. Vervolg (later, planning stap 2): inspectie automatisch toewijzen aan wie een bepaalde functiegroep heeft.
