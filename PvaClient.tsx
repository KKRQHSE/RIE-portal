    ...
    <main className="min-h-screen bg-surface">
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            <p className="font-mono text-xs text-accent uppercase tracking-widest">QVOX</p>
            <h1 className="text-xl font-semibold text-ink mt-1">{company.name}</h1>
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 mb-6 flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-sm text-ink/50">
              {afgerond} van {items.length} acties afgerond
            </p>
            {hoogOpen > 0 && (
              <p className="text-xs text-red-600 mt-1 font-medium">
                {hoogOpen} hoge prioriteit nog open
              </p>
            )}
          </div>
        </div>
